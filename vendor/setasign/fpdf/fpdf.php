...
function centreImage($img) {
    ...
    {
        list($width, $height) = $this->resizeToFits($imgPath, $containerWidth, $containerHeight);
     ...
     * @return integer
     */
    protected function pixelsToMMs(int $val) : int
    {
     ...
     * @return integer
     */
    protected function mmToPixelss(int $val) : int
    {
     ...
     * @return int[]
     */
    protected function resizeToFits(string $imgPath, int $maxWidth = 210, int $maxHeight = 297) : array
    {
        list($width, $height) = getimagesize($imgPath);
        $widthScale = $this->mmToPixelss($maxWidth) / $width;
        $heightScale = $this->mmToPixelss($maxHeight) / $height;
        $scale = min($widthScale, $heightScale);
        return array(
            $this->pixelsToMMs($scale * $width),
            $this->pixelsToMMs($scale * $height)
        );
    }
...
function Footer()
{
    ...
    $this->Cell(15,5,'Printed on:',0,0,'L');
    $this->Cell(30,5,date('M-d-Y:h:m:s'),0,0,'L');
    $this->Cell(150,5,"Page".$this->PageNo(),0,0,'R');
}
